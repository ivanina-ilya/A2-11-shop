...
export class CartService {
  ...
  inCart(sku?: string): number {
    ...
    const counter: Counter = this.getInCart(sku);
  ...
  removeFromCart(sku: string, count?: number): number {
    if ( typeof count === 'undefined' ) { count = 1; }
    const counter: Counter = this.getInCart(sku);
    if ( counter === undefined ) { throw new Error(`The product with SKU '${sku}' is missing in Cart`); }
    if ( counter.count < count ) {
      throw new Error(`The ${count} product(s) with SKU '${sku}' is not available in cart (${counter.count})`);
    }
    counter.count -= count;
    this.storeService.pushInStock(sku, count);

    if ( counter.count === 0) {
      const index = this._inCart.map(it => it.sku).indexOf(sku);
